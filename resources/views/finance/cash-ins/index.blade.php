...
@section('page_title', 'Cash In')
@section('content')
<div class="card mb-3"><div class="card-header">Record Manual Cash In</div><div class="card-body"><form method="POST" action="{{ route('finance.cash-ins.store') }}" class="form-row">@csrf
...
<div class="col-md-3"><label>Source</label><input class="form-control" name="source_type" placeholder="collections / other_income" required></div>
...
<div class="card"><div class="card-body table-responsive p-0"><table class="table table-sm mb-0"><thead><tr><th>No</th><th>Branch</th><th>Source</th><th>Amount</th><th>Received</th></tr></thead><tbody>@foreach($cashIns as $entry)<tr><td>{{ $entry->cash_in_number }}</td><td>{{ $entry->branch?->name }}</td><td>{{ $entry->source_type }}</td><td>{{ number_format($entry->amount,2) }}</td><td>{{ $entry->received_at }}</td></tr>@endforeach</tbody></table></div><div class="card-footer">{{ $cashIns->links() }}</div></div>
@endsection
