...
@php
    ...
    $selectedBranch = $branches->firstWhere('id', old('branch_id', $leaveRequest->branch_id ?: $currentUser?->primary_branch_id));
@endphp
        ...
        <div class="card-body">
            <div class="form-row">
                <div class="col-md-4 mb-3">
                    ...
                    @if ($selfService)
                        <input type="hidden" name="user_id" value="{{ old('user_id', $leaveRequest->user_id ?: $currentUser?->id) }}">
                        <input class="form-control" value="{{ $currentUser?->display_name }}" disabled>
                    @else
                        <select name="user_id" class="form-control" required>@foreach($users as $user)<option value="{{ $user->id }}" @selected((int) old('user_id', $leaveRequest->user_id) === $user->id)>{{ $user->display_name }}</option>@endforeach</select>
                    @endif
                </div>
                <div class="col-md-4 mb-3">
                    <label>Branch *</label>
                    @if ($selfService)
                        <input type="hidden" name="branch_id" value="{{ old('branch_id', $leaveRequest->branch_id ?: $currentUser?->primary_branch_id) }}">
                        <input class="form-control" value="{{ $selectedBranch?->branch_name ?? $selectedBranch?->name }}" disabled>
                    @else
                        <select name="branch_id" class="form-control" required>@foreach($branches as $branch)<option value="{{ $branch->id }}" @selected((int) old('branch_id', $leaveRequest->branch_id) === $branch->id)>{{ $branch->branch_name ?? $branch->name }}</option>@endforeach</select>
                    @endif
                </div>
